 ...
 * @LastEditTime: 2021-03-14 21:45:12
-->

| 名称        | 解释                                                                                     |
| ----------- | ---------------------------------------------------------------------------------------- |
| ceph-deploy | 集群部署节点,负责集群整体部署,也可以复用cpeh集群中的节点作为部署节点                     |
| monitor     | 监视管理节点,承担集群重要的管理任务,一般需要3或5个节点                                   |
...
Ceph 存储集群的部署都始于部署一个个 Ceph 节点、网络和 Ceph 存储集群. Ceph 存储集群至少需要一个 Ceph Monitor 和两个 OSD 守护进程.而运行 Ceph 文件系统客户端时,则必须要有元数据服务器(Metadata Server ).

## Monitors

Ceph Monitor（ceph-mon)维护集群状态的映射,包括监视器映射,管理器映射,OSD映射,MDS映射和CRUSH映射.  这些映射是Ceph守护程序相互协调所需的关键群集状态.  

监视器还负责管理守护程序和客户端之间的身份验证.通常至少需三个监视器才能实现冗余和高可用性.
...
Ceph OSD(对象存储守护程序,ceph-osd)存储数据,处理数据复制,恢复,重新平衡,并通过检查其他Ceph OSD守护程序的心跳来向Ceph监视器和管理器提供一些监视信息.
